# trigger daily incense patch v2
...
if old not in s: raise SystemExit('inc render not found')
s=s.replace(old,"if(tab==='incense')b.querySelector('#wild-content').innerHTML=dailyIncenseCards()+bonusCards(inc,'Incense');",1)
anchor="b.querySelectorAll('[data-w]').forEach(x=>x.onclick=async()=>"
if anchor not in s: raise SystemExit('wild pokemon handler not found')
s=s.replace(anchor,"b.querySelectorAll('[data-daily]').forEach(x=>x.onclick=()=>showDailyIncense());b.querySelectorAll('[data-w]').forEach(x=>x.onclick=async()=>",1)
p.write_text(s)
