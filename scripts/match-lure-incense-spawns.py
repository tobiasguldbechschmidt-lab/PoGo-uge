...
replacement = r'''async function showNormalLureSpawns(){
  ...
  try{
    ...
    let e=active.find(e=>{let d=e.details||{};return Object.keys(d).some(k=>/lure/i.test(k)&&Array.isArray(d[k])&&d[k].some(v=>v&&v.name))})||active.find(e=>Array.isArray(e.details?.lure_spawns)&&e.details.lure_spawns.length)||active.find(e=>Array.isArray(e.details?.spawns)&&e.details.spawns.length&&/lure/i.test(JSON.stringify(e)));
    if(!e){open('<h2>Lure Module</h2><div class=note>Der er ingen aktive event-spawns til Lure Module lige nu.</div>');return}
    ...
    let list=[];
    vals.forEach(v=>{let p=typeof v==='string'?{name:v}:v;if(p&&p.name&&!list.some(x=>x.name===p.name))list.push(p)});
    if(!list.length){open(`<h2>Lure Module</h2><div class=small>${E(e.title)}</div><div class=note>Ingen Lure-spawns er registreret for dette event endnu.</div>`);return}
    open(`<h2>Lure Module</h2><div class=small>${E(e.title)}</div><div class=section>Spawns</div>${list.map((p,i)=>`<div class="card click" data-lure-p="${i}"><div class=row><div class=img>${p.asset_url?`<img src="${E(p.asset_url)}" alt="${E(p.name)}" loading="lazy">`:lureImg(p.name)?`<img src="${E(lureImg(p.name))}" alt="${E(p.name)}" loading="lazy">`:'🌸'}</div><div class=grow><div class=name>${E(p.name)}</div><div class=small>${p.shiny_available===true?'✨ Shiny mulig':'Shiny ikke oplyst'}</div></div>›</div></div>`).join('')}`);
  ...
  }catch(e){open('<h2>Lure Module</h2><div class=note>Kunne ikke hente Lure-spawns. Tryk ↻ igen.</div>')}
}'''
...
for needle in ['data-lure-p=', 'Lure Module</h2>', 'showLurePokemonTypes']:
    if needle not in s:
