...
old="w.innerHTML=bonusCards(inc,'Incense');"
if old not in s: raise SystemExit('inc render not found')
s=s.replace(old,"w.innerHTML=dailyIncenseCards()+bonusCards(inc,'Incense');",1)
anchor="document.querySelectorAll('[data-w]').forEach(x=>x.onclick="
if anchor not in s: raise SystemExit('wild tab handler not found')
s=s.replace(anchor,"document.querySelectorAll('[data-daily]').forEach(x=>x.onclick=()=>showDailyIncense());document.querySelectorAll('[data-w]').forEach(x=>x.onclick=",1)
p.write_text(s)
