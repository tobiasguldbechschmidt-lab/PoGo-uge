...
s=re.sub(r"document\.addEventListener\('click',e=>\{let b=e\.target\.closest\('\[data-normal-lure-duration\]'\);if\(b\)\{e\.stopPropagation\(\);showNormalLureDuration\(\)\}\}\);",'',s)
...
s=remove_function(s,'showNormalLureDuration')
...
else:raise SystemExit('Lure tab renderer marker not found')
